...
export function buildProviders(env: Env): IProvider[] {
  const p: IProvider[] = [];
  if(env.SMS_ACTIVATE_API_KEY)p.push(new SmsActivateProvider(env.SMS_ACTIVATE_API_KEY));
  if(env.FIVE_SIM_API_KEY)p.push(new FiveSimProvider(env.FIVE_SIM_API_KEY));
  return p;
}
export interface BestProvider { provider: IProvider; priceUsd: number; priceNgn: number; stock: number; }
export async function findBestProvider(providers: IProvider[], countryCode: string, serviceSlug: string, exchangeRate: number): Promise<BestProvider|null> {
  ...
  return{provider:best.provider,priceUsd:best.price.priceUsd,priceNgn:Math.ceil(best.price.priceUsd*exchangeRate),stock:best.price.stock};
}
