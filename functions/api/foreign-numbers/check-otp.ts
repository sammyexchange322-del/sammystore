...
export const onRequestGet:PagesFunction<Env>=async({request,env})=>{
  ...
  const{data:order}=await supabase.from('fn_orders').select('*,fn_providers(slug)').eq('id',orderId).eq('user_id',userId).single();
  ...
  if(order.expires_at&&new Date(order.expires_at)<new Date()){await supabase.from('fn_orders').update({status:'expired'}).eq('id',orderId);return jsonResponse({status:'expired'});}
  if(order.status!=='active'||!order.provider_order_id)return jsonResponse({status:order.status});
  const providers=buildProviders(env);
  const slug=(order.fn_providers as{slug:string}|null)?.slug;
  const provider=providers.find(p=>p.slug===slug);
  if(!provider)return errorResponse('Provider not found',500);
  try{
    const sms=await provider.checkSms(order.provider_order_id);
    if(sms){
      const otp=sms.otpCode??extractOtp(sms.text);
      await supabase.from('fn_orders').update({otp_code:otp,status:'otp_received',otp_received_at:sms.receivedAt}).eq('id',orderId);
      await supabase.from('fn_otp_messages').insert({order_id:orderId,raw_text:sms.text,otp_code:otp,received_at:sms.receivedAt});
      return jsonResponse({status:'otp_received',otp_code:otp,phone_number:order.phone_number});
    }
  }catch(e){await supabase.from('fn_api_logs').insert({order_id:orderId,action:'check_sms',error:e instanceof Error?e.message:'Unknown'});}
