...
export const onRequestPost:PagesFunction<Env>=async({request,env})=>{
  ...
  const{data:order}=await supabase.from('fn_orders').select('*,fn_providers(slug)').eq('id',order_id).eq('user_id',userId).single();
  if(!order)return errorResponse('Order not found',404);
  if(!['pending','active'].includes(order.status))return errorResponse(`Cannot cancel: ${order.status}`);
  if(order.provider_order_id){
    const slug=(order.fn_providers as{slug:string}|null)?.slug;
    const provider=buildProviders(env).find(p=>p.slug===slug);
    if(provider)try{await provider.cancelOrder(order.provider_order_id);}catch{}
  }
  await supabase.rpc('fn_refund_order',{p_order_id:order_id,p_reason:'User cancelled'});
